...
func TestCertificateCreation(t *testing.T) {
	...
	ca, cert, key, err := CreateSSLConfig([]string{"localhost"}, time.Hour)
	...
	parsedCert, err := x509.ParseCertificate(c.Certificate[0])
	if err != nil {
		t.Fatal(err)
	}

	if parsedCert.NotAfter.Sub(parsedCert.NotBefore) != time.Hour {
		t.Fatal("certificate duration is not 1 hour")
	}
